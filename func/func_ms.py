...
def creat_doc_pas_gen(adres_invertory: str, adres_genotyping:str, adres) -> pd.DataFrame:
    ...
    root = tk.Tk()
    root.lift()
    root.title('Введите название хозяйства')
    ...
    for i in range(len(df)):
        ...
        #df_animal_prof.iloc[0,5] = 'f/f'
        #display(df_animal_prof)
        #display(df_fater_prof)
                ...
                if value_fater_ms != '-':
                    if locus_f in df_animal_prof.columns:
                        try:
                            ...
                            #print(value_animal_ms_loc, value_fater_ms_loc)
                            ...
                            pass
                            #print(f'У этого животного нет профиля {anmal_num}')
            except:
                pass
        
                #print(f'У этого отца нет профиля {fater_num}')
    res_error = pd.DataFrame({'number': number,
                            'locus': locus_str_er,
                            'fater': fater_num,
                            'animal': animal,
                            })
    (res_error.to_csv(r'func\data\creat_pass_doc\res_error.csv',
                sep=";",
                decimal=',',
                encoding = "cp1251"))
    files_list = []
    for i in range (len(series_num)):
        ...
        if  series_num[i] in series_proba:
            ...
            #number_animal = int(number_animal)
